...
Distribution uses the native Debian package and source installer. Release **1.1.1** uses tag **v1.1.1** and is a regular stable release.

## Prepare the version

Set `VERSION = '1.1.1'` and `RELEASE_TAG = 'v1.1.1'` in `app_info.py` before
...
Commit the reviewed source and documentation, tag the same commit `v1.1.1`, and
